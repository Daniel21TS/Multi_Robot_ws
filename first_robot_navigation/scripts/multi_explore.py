...
#---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def map_callback(self, msg):
        ...
        # Calculate Size of frontier
        frontier_robot = self.calculate_cluster_sizes(cluster_points)

        # Calculate Distance of frontier
        frontier_robot = self.calculate_distance_frontier(frontier_robot)

        # Calculate Gain Information based of RBPF of frontier
        #frontier_robot = self.calculate_information_gain(msg,frontier_robot)

        # Calculate Connectivity of frontier
        #frontier_robot = self.calculate_connectivity(frontier_robot)

        # Calculate distance from actual fp of others robots
        #frontier_robot = self.robot_avoidance(frontier_robot)
        ...
        print("Frontier " + self.robot_namespace + " after :")
    ...
    def robot_avoidance(self, frontier_robot):    
        updated_frontier_robot = []
        for size, distance, middle_point in frontier_robot:
            robot_avoidance = self.calculate_robot_avoidance(middle_point, self.robot_namespace)
            frontier_data = (size, distance, middle_point, robot_avoidance)
            updated_frontier_robot.append(frontier_data)

        return updated_frontier_robot
    
    def calculate_robot_avoidance(self, middle_point, namespace):
        robot_avoidance = 0
        distances = []
        
        for robot_goal in self.robot_goals:
            if robot_goal[0] != namespace:
                x1 = robot_goal[1]
                y1 = robot_goal[2]
                x2 = middle_point.point.x
                y2 = middle_point.point.y
        ...
        if distances == []:
            ...
            # Escolhe a menor distancia, pois o que iporta é o que esta mais perto!
            low_distance = min(distances)
            if low_distance == 0:
                robot_avoidance = 0
            else:
                # Temos que dividir 1/low_distance, pois qunato mais perto está, maior é o valor de robot avoidance!
                robot_avoidance = 1/low_distance

        return robot_avoidance
